...
//since we only need the number of shops from which we can buy
//we can sort the shop prices
//now for i th day, we have some money
//we can buy from shops where money>= selling price
//so we just do upper bound to find the shop from which we can't buy
...
int32_t main()
{
	...
	while(test--)
	{
		...
		for(int i=0;i<q;i++){
		    int money;
		    cin >> money;
		    //index of first shop from which we can't buy
		    int index = upper_bound(a.begin(),a.end(),money)-a.begin();
		    int ans=index;
		    if(ans>n){
		        cout << n << endl;
		    }
