...
// first number divisible by n will be n 
// so first n-1 numbers will be not divisible by n 
// then the next n-1 numbers will be not divisible similarly till 2n
// so we need to find how many times k is divisible by n-1 
// that will represent the numbers we need to skip

// but if k is exactly divisible by n-1, then that represents
// the n-1 multiple, so we need to take one number back
