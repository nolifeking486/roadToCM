...
// logic, implementation, stack
// no need to actually create the regular expression
// since we have the freedom to either place the character at the back or the front 
// we can assume all open brackets incorrectly placed to be moved to the front 
// all closing brackets to be moved to the back 
// so barring all regular brackets only the moves needed to be made will remain
// hence we only need to check for regularity in one iteration using stack
